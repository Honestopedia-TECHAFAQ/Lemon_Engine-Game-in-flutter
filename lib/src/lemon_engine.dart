// ignore_for_file: must_be_immutable, non_constant_identifier_names, constant_identifier_names, unnecessary_this, no_leading_underscores_for_local_identifiers, unnecessary_brace_in_string_interps, unused_import
...
import 'package:app/src/math.dart';
...
abstract class LemonEngine extends StatelessWidget {
  ...
  /// override safe
  Widget buildLoadingPage(BuildContext context) => const Text("LOADING");
  /// override safe
  ...
  TextSpan getTextSpan(String text) {
    ...
    value = TextSpan(style: const TextStyle(color: Colors.white), text: text);
  ...
  void renderCircleFilled({
    ...
    const angle = (2 * 3.14159) / _renderCircleSegments;
    var j = 0;
      ...
      _internalInit().catchError((error){
        app.value = MaterialApp(
          ...
          home: Scaffold(
            ...
            body: Center(child: Text(error.toString(), style: const TextStyle(color: Colors.white))),
          ),
        );
      });
    } else {
...
// global typedefs
typedef DrawCanvas = Function(Canvas canvas, Size size);
